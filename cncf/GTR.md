...
* How does the project clean up any resources created, including CRDs?
  ...
  **ManifestWork cleanup — the project provides tooling to prevent orphaning.** `clusteradm unjoin` (the recommended spoke removal command) proactively checks for `AppliedManifestWork` resources on the spoke before proceeding. If any exist, it refuses to continue and warns the operator:

  > *"appliedManifestWorks still exist on the managed cluster, you should manually clean them, uninstall klusterlet will cause those works out of control."*
  ...
  The recommended sequence is always: delete `ManagedCluster` from the hub first (triggering `ResourceCleanup` to clean spoke resources), then run `clusteradm unjoin`. Direct `Klusterlet` CR deletion is technically possible but not advised and unsupported at scale.
...
* Explain how upgrades and rollbacks were tested and how the upgrade-\>downgrade-\>upgrade path was tested.  
  ...
  OCM components (ClusterManager and Klusterlet) are upgraded via `clusteradm upgrade` or by updating the image tag in the operator Deployment. The project follows semver conventions, and the API upgrade flow is documented at [API upgrade flow](https://github.com/open-cluster-management-io/api/blob/main/docs/development.md#api-upgrade-flow). Downgrade is supported by specifying a lower version target with the same tooling.

  In-flight workloads are not affected by hub upgrades — spokes continue running existing ManifestWork-deployed resources during the upgrade window, consistent with OCM's graceful degradation behavior.
...
* How can an operator determine if the project is in use by workloads?
  ...
  Resources applied to spokes via ManifestWork carry an `ownerReference` back to their `AppliedManifestWork`. The work agent deliberately uses `ownerReference` rather than `controllerReference` to support shared ownership — multiple ManifestWorks can co-own a resource on the spoke. Any resource on a spoke with an `ownerReference` of kind `AppliedManifestWork` is under OCM management, allowing operators to audit OCM-managed resources on a spoke independently of the hub. See [ManifestWork](https://open-cluster-management.io/docs/concepts/manifestwork/).
...
* How does this project recover if a key component or feature becomes unavailable? e.g Kubernetes API server, etcd, database, leader node, etc.
  ...
  When the hub becomes unavailable, managed clusters continue running existing workloads — there is no immediate workload impact due to OCM's pull-based graceful degradation.

  For hub recovery, OCM custom resource data (ManagedCluster, ManifestWork, Placement, etc.) should be backed up and can be restored to a new control plane, with Klusterlet agents reconfigured to reconnect to the new hub endpoint. The recovery procedure is documented in the [multiple hubs solution](https://github.com/open-cluster-management-io/ocm/tree/main/solutions/multiplehubs), which covers hub replacement and agent reconnection.
